...
from .exceptions import SpotifyServiceError
...
class SpotifyService:
    ...
    def __init__(self):
        self.client_id = config("SPOTIFY_CLIENT_ID", default=None)
        self.client_secret = config("SPOTIFY_CLIENT_SECRET", default=None)
        ...
        if not self.client_secret:
            logger.error(
                "SPOTIFY_CLIENT_SECRET não encontrada nas variáveis de ambiente"
            )
        ...
        try:
            ...
            logger.info("Spotify service initialized successfully (Client Credentials)")
        except Exception as e:
            logger.error(f"Error initializing Spotify service: {e}")
            raise SpotifyServiceError(
                f"Failed to initialize Spotify service: {e}"
            ) from e
    ...
    def get_artist_albums(self, max_results: int = 50) -> List[Dict]:
        ...
        try:
            ...
            search_results = self.spotify.search(
                q=f'artist:"{self.artist_name}"', type="artist", limit=1
            )
            ...
            logger.error(f"Error fetching albums for {self.artist_name}: {e}")
            raise SpotifyServiceError(f"Failed to fetch albums: {e}") from e
    ...
    def get_album_details(self, album_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            raise SpotifyServiceError(f"Failed to fetch album details: {e}") from e
    ...
    def search_albums(self, query: str, max_results: int = 20) -> List[Dict]:
        ...
        logger.info(
            f"Encontrados {len(filtered_albums)} álbuns do {self.artist_name} para a busca '{query}'"
        )
...
def get_spotify_service() -> SpotifyService:
    ...
    if spotify_service is None:
        try:
            ...
            logger.error(f"Error creating Spotify service: {e}")
            raise SpotifyServiceError(f"Failed to create Spotify service: {e}") from e
