async function loadSpotifyPlaylists() {
  try {
    ...
    playlists.forEach(({ playlistId, title, url }) => {
      ...
      col.className = "text-center mb-2.5";
      ...
      col.innerHTML = `
          ...
          <div class="playlist-links">
            <a href="${url}" class="btn spotify" target="_blank">
...
function filterSpotifyPlaylists(searchTerm) {
  const items = document.querySelectorAll("#spotifyContainer .text-center");
  items.forEach((item) => {
    const title = item
      .querySelector(".playlist-links .btn")
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  loadSpotifyPlaylists().then(() => {
    ...
    const cards = document.querySelectorAll("#spotifyContainer .text-center");
