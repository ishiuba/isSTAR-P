...
class YouTubeService:
    ...
    def get_channel_playlists(self, max_results: int = 50) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Erro inesperado ao buscar playlists: {e}")
            return []
    ...
    def get_playlist_details(self, playlist_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Erro inesperado ao buscar playlist {playlist_id}: {e}")
            return None
    ...
    def search_playlists(self, query: str, max_results: int = 20) -> List[Dict]:
        ...
        for playlist in all_playlists:
            ...
            tags_match = any(query_lower in tag.lower() for tag in playlist.get('tags', []))
        ...
        logger.info(f"Encontradas {len(filtered_playlists)} playlists para a busca '{query}'")
    ...
    def get_featured_playlist(self) -> Optional[Dict]:
        ...
        featured = max(playlists, key=lambda p: (
            p.get('video_count', 0),
            p.get('published_at', '')
        ))
    ...
    def get_channel_statistics(self) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"Estatísticas do canal carregadas: {stats_data['view_count']} views, {stats_data['subscriber_count']} subscribers")

            return stats_data

        except HttpError as e:
            logger.error(f"Erro HTTP ao buscar estatísticas do canal: {e}")
            return None
        except Exception as e:
            logger.error(f"Erro ao buscar estatísticas do canal: {e}")
            return None
