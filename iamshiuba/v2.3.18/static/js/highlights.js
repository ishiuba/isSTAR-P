...
class HighlightManager {
  ...
  setup() {
    ...
    // Read playlist ID from data attribute
    this.playlistId = this.container.dataset.playlistId;

    if (!this.playlistId) {
      console.error("No playlist ID provided in data attribute");
      return;
    }
   ...
   */
  renderHighlight() {
    if (!this.container || !this.highlightElement || !this.playlistId) return;

    // Update iframe attributes
    const iframe = this.highlightElement.querySelector("iframe");
    if (iframe) {
      iframe.src = `https://www.youtube.com/embed/videoseries?list=${this.playlistId}`;
